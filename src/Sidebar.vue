...
<aside class="sm-side">
    ...
    <div class="user-head">
        <img src="src/assets/images/profile.jpg" alt="profile pic">
...
export default {
    ...
    created() {
        eventBus.$on('changeView', ({ tag }) => this.activeView = tag)
    },
    props: {
        messages: {
            type: Array,
    ...
    computed: {
        unreadMessages() {
            return this.messages.filter(message => message.type === 'incoming' && !message.isRead && !message.isDeleted)
        },
        sentMessages() {
            return this.messages.filter(message => message.type === 'outgoing' && !message.isDeleted)
        },
        importantMessages() {
            return this.messages.filter(message => message.type === 'incoming' && message.isImportant && !message.isDeleted)
        },
        trashedMessages() {
            return this.messages.filter(message => message.isDeleted)
        },
    },
    methods: {
